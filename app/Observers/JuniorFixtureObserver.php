        ...
        if ($unplayedGame == null) {
            ...
            if ($juniorFixture->stage < 3) {
                for ($group = 1; $group < 4; $group++) {
                    $teamToPromote = JuniorLeagueTable::where('competitionID', $juniorFixture->competitionID)
                        ->where('stage', $juniorFixture->stage)
                        ->where('group', $group)
                        ->orderBy('points', 'DESC')->orderBy('bilans', 'DESC')
                        ->first();

                    $teamToRelegate = JuniorLeagueTable::where('competitionID', $juniorFixture->competitionID)
                        ->where('stage', $juniorFixture->stage)
                        ->where('group', $group)
                        ->orderBy('points', 'ASC')->orderBy('bilans', 'ASC')
                        ->first();
